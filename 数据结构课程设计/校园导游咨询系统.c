...
void Create_Map(mgraphtype *g)
{
	...
	if (rf)
	{
		fscanf(rf, "%d%d", &g->vexNum, &g->edgNum);//读取节点数 边数
		for (i = 0; i < g->vexNum; i++)
			fscanf(rf, "%d%s%s", &g->vexs[i].num, g->vexs[i].name, g->vexs[i].intro);  //输入顶点信息创建  顶点表
		for (i = 0; i < g->vexNum; i++)//初始化邻接矩阵
			for (j = 0; j < g->vexNum; j++)
			{
				if (i == j)
					g->edge[i][j] = 0;//自己到自己的权值为0
				else
					g->edge[i][j] = INFINITY; ///初始化顶点之间的权值 默认为无穷
			}
		for (k = 0; k < g->edgNum; k++)
		{
			fscanf(rf, "%d%d%d", &i, &j, &e); //读取值  对邻接表进行赋值
			g->edge[i][j] = g->edge[j][i] = e;//有向图时不需要这个，无向图时需要加上
		}
		fclose(rf);
	}
	else
...
void Dis_Map() {
	...
	printf("                       ║        8.马克思学院◎--║                                                                                     \r");
	printf("                       ║                        ║                                                                                     \r");
	printf("                       ║        7.计算机中心◎--║----◎4.篮球场                                                                     \r");
	printf("        6.校史馆◎----------------------------- ║    ║                                                                                \r");
	printf("                ║                   3.图书馆◎--║    ║                                                                               \r");
	printf("                ║                               ║----◎2.11号教学楼                                                                   \r");
	printf("                ║                               ║    ║                                                                               \r");
	printf("      5.西区天桥◎-----------------------------------◎1.中校区门口                                                                     \r");
...
void ShortPath(mgraphtype *g) {
	...
	for (i = 0; i < g->vexNum; i++)                                /*初始化距离向量矩阵与路径向量矩阵*/
		for (j = 0; j < g->vexNum; j++) {
			dist[i][j] = g->edge[i][j];// "顶点i"到"顶点j"的路径长度为"i到j的权值"
			if (i != j && dist[i][j] < INFINITY) path[i][j] = i; //"顶点i"到"顶点j"的最短路径是经过顶点j
			else path[i][j] = -1;                                  /*-1代表当前两点不可达*/
		}
	for (k = 0; k < g->vexNum; k++)                                /*递推求解每两建筑的最短路径*/
		for (i = 0; i < g->vexNum; i++)
...
void Floyd_Print(mgraphtype *g, int sNum, int eNum) {
	if (path[sNum][eNum] == -1 || path[sNum][eNum] == eNum || path[sNum][eNum] == sNum)
		...
		printf("%s->", g->vexs[path[sNum][eNum]].name);         /*打印中间建筑名字*/
...
void Allpath_Print(mgraphtype *g) {
	...
	count = 1;                                                       /*路径计数器*/
	top = 0;                                                         /*栈顶*/
	memset(pathStack, 0, sizeof(pathStack));                         /*路径栈初始化   memset() 函数常用于内存空间初始化*/
...
int menu() {
	int s;
	printf("\t\t            ┌------------------------------------------┐           \n");
	printf("\t\t           ├---- 欢迎使用齐齐哈尔大学校园导游咨询系统 --┤          \n");
	printf("\t\t            └------------------------------------------┘           \n");
	printf("\t\t                                                                      \n");
	printf("\t\t                                                                      \n");
	printf("\t\t   ┌------------------------┐        ┌------------------------┐   \n");
	printf("\t\t   │ 1.齐大中校区建筑信息   │        │ 2.两建筑间最短路径查询 │   \n");
	printf("\t\t   └------------------------┘        └------------------------┘   \n");
	printf("\t\t                                                                      \n");
	printf("\t\t   ┌------------------------┐        ┌------------------------┐   \n");
	printf("\t\t   │ 3.两建筑间所有路径查询 │        │ 4.多建筑间访问路线查询 │   \n");
	printf("\t\t   └------------------------┘        └------------------------┘   \n");
	printf("\t\t                                                                      \n");
	printf("\t\t   ┌------------------------┐        ┌------------------------┐   \n");
...
{
	int close;                                               //建筑关闭的标记
	char name[20];                                           //存放建筑的名字
	char reason[100];                                        //存放建筑关闭的原因
}Scenic_Manager[M];
...
void ScenicManager(mgraphtype *g)
{
	...
	else
	{
		...
		strcpy(Scenic_Manager[no].name, g->vexs[no].name);//将后者复制给前者
		printf("\n");
		printf("请输入建筑关闭的原因: \n");
...
void main() {
	...
	if (Login_System()) {
		while (!quit) {                                                          /*系统退出条件满足判定*/
			switch (menu()) {                                                    /*打印主菜单*/
			case 1:system("cls"); Dis_Map(); Search_Location(&g); break;         /*齐齐哈尔大学校园建筑信息查询*/
			case 2:system("cls"); Dis_Map(); Shortpath_Print(&g); break;         /*两建筑间最短路径查询*/
			case 3:system("cls"); Dis_Map(); Allpath_Print(&g); break;           /*两建筑间所有路径查询*/
			case 4:system("cls"); Dis_Map(); BestPath(&g); break;                /*多建筑间访问路线查询*/
